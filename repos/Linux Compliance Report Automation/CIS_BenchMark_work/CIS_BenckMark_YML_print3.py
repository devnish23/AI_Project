...
def parse_cis_benchmark(pdf_path):
    ...
    try:
        ...
        print(f"Parsed {len(benchmark_rules)} rules from {pdf_path}")
        return benchmark_rules
    except Exception as e:
        error_message = f"Error parsing {pdf_path}: {str(e)}"
        print(error_message)
...
"""
    ...
    print(f"Generated Ansible YAML file: {yaml_file}")
    return yaml_file
...
def generate_jinja_template(output_dir):
    ...
    print(f"Generated Jinja2 template file: {jinja_file}")
    return jinja_file
...
def process_benchmarks():
    ...
    for pdf_path in file_paths:
        ...
        if isinstance(benchmark_rules, dict) and "Error" in benchmark_rules:
            messagebox.showerror("Error", benchmark_rules["Error"])
            print(benchmark_rules["Error"])
            continue
        ...
        print(f"  - Jinja2 Template: {jinja_file}")
