...
def parse_cis_benchmarks(pdf_text):
    benchmarks = []
...
def generate_ansible_yaml(benchmarks, output_path):
    try:
        yaml_content = []
...
def select_and_convert_files():
    ...
    for pdf_path in file_paths:
        ...
        benchmarks = parse_cis_benchmarks(pdf_text)
        if not benchmarks:
            print(f"No benchmarks found in {pdf_path}")
            continue

        # Generate YAML output
        output_path = os.path.splitext(pdf_path)[0] + ".yml"
        generated_path = generate_ansible_yaml(benchmarks, output_path)
        if generated_path:
